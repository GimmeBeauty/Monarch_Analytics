...
def rebuild_ad_summaries():
    ...
    cur = conn.cursor()
    cur.execute("DELETE FROM MONARCH_RAW.ADS.DAILY_AD_SUMMARY WHERE summary_date >= DATEADD(day,-3,CURRENT_DATE()) AND channel IN ('meta_ads','google_ads','pinterest_ads','criteo_ads')")
