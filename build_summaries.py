...
print("Done")
print("\nBuilding Amazon Ads summary...")
cur.execute("INSERT INTO MONARCH_RAW.ADS.DAILY_AD_SUMMARY (summary_date,channel,spend,impressions,clicks,conversions,conversion_value,ctr,cpc,cpm,roas) SELECT ad_date,'amazon_ads',SUM(spend),SUM(impressions),SUM(clicks),SUM(conversions),SUM(ad_revenue),CASE WHEN SUM(impressions)>0 THEN SUM(clicks)/SUM(impressions) ELSE 0 END,CASE WHEN SUM(clicks)>0 THEN SUM(spend)/SUM(clicks) ELSE 0 END,CASE WHEN SUM(impressions)>0 THEN SUM(spend)/SUM(impressions)*1000 ELSE 0 END,CASE WHEN SUM(spend)>0 THEN SUM(ad_revenue)/SUM(spend) ELSE 0 END FROM MONARCH_RAW.ADS.AMAZON_ADS_RAW GROUP BY ad_date")
print("Done")
