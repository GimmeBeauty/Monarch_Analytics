...
function ContribBar({ value, color }: { value: number; color: string }) {
  return (
    <div className="w-16 h-1.5 rounded-full bg-[#3A3A3A]/8 dark:bg-[#003349]/8 overflow-hidden">
      <div
...
function TopStores({ stores }: { stores: StoreBreakdown[] }) {
  return (
    <div className={`rounded-2xl p-5 ${CARD_STYLE}`}>
      <div className="mb-4">
        <h3 className="text-sm font-semibold text-[#3A3A3A] dark:text-[#003349]">Top Stores</h3>
        <p className="text-xs text-[#3A3A3A]/45 dark:text-[#003349]/35 mt-0.5">Revenue by store</p>
      </div>
      ...
      <div className="space-y-3">
        {stores.map((store) => (
          <div key={store.storeId} className="flex items-center gap-3">
            ...
            {/* Store name */}
            <span className="text-xs text-[#3A3A3A]/70 dark:text-[#003349]/60 w-24 truncate shrink-0">
              {store.label}
            </span>
            ...
            {/* Revenue */}
            <span className="text-xs font-semibold text-[#3A3A3A] dark:text-[#003349] tabular-nums w-16 text-right shrink-0">
              {store.formattedRevenue}
            </span>

            {/* Contribution % */}
            <span className="text-xs text-[#3A3A3A]/40 dark:text-[#003349]/30 tabular-nums w-10 text-right shrink-0">
  ...
  return (
    <div className={`rounded-2xl p-5 ${CARD_STYLE}`}>
      <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
        <div>
          <h3 className="text-sm font-semibold text-[#3A3A3A] dark:text-[#003349]">Top Channels</h3>
          <p className="text-xs text-[#3A3A3A]/45 dark:text-[#003349]/35 mt-0.5">Across mapped channels</p>
        </div>
        <div className="flex items-center gap-1 p-0.5 rounded-lg bg-[#3A3A3A]/5 dark:bg-[#003349]/5">
            ...
            <button
              ...
              className={`px-2.5 py-1 rounded-md text-xs font-medium transition-all ${
                view === v
                  ...
                  : "text-[#3A3A3A]/50 dark:text-[#003349]/40 hover:text-[#3A3A3A]/80"
              }`}
            >
      ...
      <div className="space-y-3">
        {sorted.slice(0, 7).map((ch) => (
          <div key={ch.channelId} className="flex items-center gap-3">
            ...
            <span className="text-xs text-[#3A3A3A]/70 dark:text-[#003349]/60 w-28 truncate shrink-0">
              {ch.label}
            </span>
            <div className="flex-1 min-w-0">
...
function PieTooltip({ active, payload }: TooltipProps<number, string>) {
  ...
  return (
    <div className="rounded-xl border border-[#FFBC80]/30 dark:border-[#9BDBF3]/30 bg-white/95 dark:bg-[#FFFFFF]/95 shadow-lg px-3 py-2">
      <p className="text-xs font-semibold text-[#3A3A3A] dark:text-[#003349]">{item.name}</p>
      <p className="text-xs text-[#3A3A3A]/60 dark:text-[#003349]/50 tabular-nums">{(item.value as number).toFixed(1)}%</p>
    </div>
  );
}
...
function ContributionBreakdown({
  ...
  return (
    <div className={`rounded-2xl p-5 ${CARD_STYLE}`}>
      <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
        <div>
          <h3 className="text-sm font-semibold text-[#3A3A3A] dark:text-[#003349]">Contribution</h3>
        ...
        <div className="flex-1 min-w-0 space-y-2">
          {slices.map((slice, i) => (
            <div key={i} className="flex items-center gap-2">
              ...
              <span className="text-xs text-[#3A3A3A]/65 dark:text-[#003349]/55 truncate flex-1 min-w-0">
                {slice.name}
              </span>
              <span className="text-xs font-semibold text-[#3A3A3A] dark:text-[#003349] tabular-nums shrink-0">
