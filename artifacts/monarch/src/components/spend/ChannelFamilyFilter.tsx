...
const FAMILY_CONFIG: Record<ChannelFamily, {
  ...
  core: {
    ...
    inactiveClasses: "border-[#3A3A3A]/15 dark:border-[#003349]/10 text-[#3A3A3A]/45 dark:text-[#003349]/35 hover:border-[#3A3A3A]/30",
  },
  rmn: {
    label: "Retail Media",
  ...
  experimental: {
    ...
    icon: FlaskConical,
    activeClasses: "border-purple-300 dark:border-purple-300 bg-purple-50 dark:bg-purple-100 text-purple-700 dark:text-purple-700",
    inactiveClasses: "border-[#3A3A3A]/15 dark:border-[#003349]/10 text-[#3A3A3A]/45 dark:text-[#003349]/35 hover:border-[#3A3A3A]/30 opacity-60",
  },
};
...
function ViewBadge({ mode, channelCount, totalCount }: { mode: ViewMode; channelCount: number; totalCount: number }) {
  ...
  return (
    <div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-semibold ${
      isTotal  ? "bg-[#FFBC80]/20 dark:bg-[#EFBAE1]/20 text-[#3A3A3A] dark:text-[#9BDBF3] border border-[#FFBC80]/40 dark:border-[#9BDBF3]/40" :
      isCustom ? "bg-purple-50 dark:bg-purple-100 text-purple-700 dark:text-purple-700 border border-purple-200 dark:border-purple-300" :
                 "bg-[#3A3A3A]/6 dark:bg-[#003349]/6 text-[#3A3A3A]/60 dark:text-[#003349]/50 border border-[#3A3A3A]/12 dark:border-[#003349]/10"
    }`}>
      <div className={`w-1.5 h-1.5 rounded-full ${
        isTotal ? "bg-[#FFBC80] dark:bg-[#BFA1E3]" : isCustom ? "bg-purple-500" : "bg-[#3A3A3A]/40 dark:bg-[#003349]/40"
      }`} />
...
export default function ChannelFamilyFilter({ state, channelCounts, onChange }: ChannelFamilyFilterProps) {
  ...
  return (
    <div className="rounded-2xl border border-[#FFBC80]/25 dark:border-[#9BDBF3]/25 bg-white dark:bg-[#FFFFFF] px-5 py-3.5 flex flex-wrap items-center gap-4">
      {/* Left: icon + scope label */}
      <div className="flex items-center gap-2 shrink-0">
        <Layers className="w-4 h-4 text-[#FFBC80] dark:text-[#BFA1E3]" />
      ...
      {/* View mode shortcuts */}
      <div className="flex items-center gap-1 p-0.5 rounded-lg bg-[#3A3A3A]/5 dark:bg-[#003349]/5 shrink-0">
          ...
          <button
            ...
            className={`px-3 py-1 rounded-md text-xs font-semibold transition-all ${
              (viewMode === mode || (mode === "total" && viewMode === "custom" && enabledFamilies.size > 1))
                ...
                : "text-[#3A3A3A]/45 dark:text-[#003349]/35 hover:text-[#3A3A3A]/70"
            }`}
          >
      ...
      {/* Divider */}
      <div className="w-px h-5 bg-[#FFBC80]/25 dark:bg-[#9BDBF3]/25 shrink-0 hidden sm:block" />
          ...
          return (
            ...
            >
              ...
              <span className={`text-[10px] px-1 rounded ${
                active ? "bg-white/40 dark:bg-black/20" : "bg-[#3A3A3A]/6 dark:bg-[#003349]/6"
              }`}>
                {count}
              </span>
            </button>
      ...
      <div className="ml-auto flex items-center gap-2 shrink-0">
        ...
        <div
          className="w-5 h-5 rounded-full flex items-center justify-center text-[#3A3A3A]/30 dark:text-[#003349]/20 hover:text-[#3A3A3A]/60 cursor-help transition-colors"
